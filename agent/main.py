...
logger = logging.getLogger("Bright Hikari-worker")
...
def run_multimodal_agent(ctx: JobContext, participant: rtc.Participant):
    ...
    if config.modalities == ["text", "audio"]:
        session.conversation.item.create(
            llm.ChatMessage(
                role="user",
                content="Please begin the interaction with the user in a manner consistent with your instructions. Say hello to user to begin the interaction.",
            )
        )
